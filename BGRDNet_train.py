...
from models.BGRDNet import BGRDNet
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            if i % 100 == 0 or i == total_step or i==1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:.4f} Loss4: {:.4f} Loss5: {:.4f} Loss6: {:.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss1.data, loss2.data, loss3.data, loss4.data, loss5.data, loss6.data))
                logging.info('#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:.4f} Loss3: {:.4f} Loss4: {:.4f} Loss5: {:.4f} Loss6: {:.4f}'.
                    format( epoch, opt.epoch, i, total_step, loss1.data, loss2.data, loss3.data, loss4.data, loss5.data, loss6.data))
...
def test(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            image = image.cuda()
            depth = depth.cuda()
            res,_,_,_,_,_ = model(image,depth)
